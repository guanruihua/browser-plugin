...
import { FORM_LIST, CONFIG_LIST } from '../config'
...
function Index(props: any) {
  const { formList = [], setFormList, setConfigList }: any = props
  ...
  const UpdateListKeyValue = (id: string, key: string, value: string): void => {
    ...
    tmp_config_list &&
      ...
      tmp_config_list.map((item: any): any => {
        if (item && item.id === id) {
          item.formList = JSON.stringify(list) || '[]'
        }
        return item
      })
  ...
  return (
    ...
    >
      <button onClick={(): void => addFormListItem()}>+</button>
      {formList &&
