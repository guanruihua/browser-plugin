...
const getStr = (len: number) => {
  const item = `_${len}_`

  return new Array(Math.ceil(len / item.length)).fill(item).join('').slice(0, len)
}
...
export function Gen() {
  const [len, setLen] = React.useState(32)
  ...
  const Row = ({ len }: any) => {
    const value = getStr(len)
    return (
      <div className='row'>
        <span className='value'>{value}</span>
        <span className='value len'>{`<${len}>`}</span>
        <span
          className='icon'
          onClick={() => {
            copyText(value) && setTime(getNow() + 3000)
          }}
        >
          {CopyIcon}
        </span>
    ...
    <div className='panel-gen'>
      ...
      <div>
        <input
          ...
          type='number'
          onChange={(e: any) => {
            setLen(Number(e.target.value) ?? 1)
          }}
          maxLength={6}
          min={1}
        />
