...
export function Tool() {
  const { activeTab, tabs, pin, pinAll, clone, mute, muteAll, ...h } = useHook()
  ...
  return (
    <Grid columns={1} className='popup-tool'>
      ...
      <Grid columns={2}>
        <Button onClick={() => h.sameDomainReopen()}>同域名重新打开 </Button>
      ...
      <Grid columns={2}>
        <Button onClick={() => h.night()}>Night</Button>
        <Button onClick={() => h.cancelNight()}>Cancel Night</Button>
      </Grid>
      {/* {isEffectArray(tabs) &&
        tabs.map((item: any, i) => {
          const { title } = item
